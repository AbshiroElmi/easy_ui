...
def get_context(context):
	...
	include_head_js = hooks.get("app_include_head_js", [])
	...
	context.update ({
		"app_path": hooks.get("app_path", '')[0],
			...
			"include_head_js": include_head_js,
			...
			"desk_theme": boot.get("desk_theme") or "easytouch",
			"csrf_token": csrf_token,
			"google_analytics_id": frappe.conf.get("google_analytics_id"),
			"google_analytics_anonymize_ip": frappe.conf.get("google_analytics_anonymize_ip"),
			"app_name": (
				frappe.get_website_settings("app_name") or frappe.get_system_settings("app_name") or "EasyTouch"
			),
    })
	context["title"] = "Login"
